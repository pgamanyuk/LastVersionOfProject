document.addEventListener("DOMContentLoaded", (e) => {
    ...
    const settingsLink = document.querySelectorAll(".settings__link");
    const fadePage = document.querySelector('.fade__page');
    const body = document.querySelector('body');

    settingsLink.forEach((link,index) => {
        link.addEventListener('click', (e) => {
            settingsLink.forEach(item => {
                item.classList.remove('signbtn-active');
            })
            settingsLink[index].classList.add('signbtn-active');
        })
    })
